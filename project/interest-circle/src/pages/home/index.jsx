...
const Home = () => {
    ...
    useEffect(() => {
        const get_circles = async () => {
            try {
                const response = await axios.get('http://localhost:8080/circle/get_circles');
                        ...
                        <div className="relative flex flex-col min-w-0 break-words border border-dashed bg-clip-border rounded-2xl border-stone-200 bg-light/30">
                            ...
                            <div className="px-9 pt-5 flex justify-between items-stretch flex-wrap min-h-[70px] pb-0 bg-transparent">
                                <h3 className="flex flex-col items-start justify-center m-2 ml-0 font-medium text-xl/tight text-dark">
                                    <span className="mr-3 font-semibold text-dark">所有圈子</span>
                                    <span className="mt-1 font-medium text-secondary-dark text-lg/normal">在这里选择感兴趣的圈子加入吧</span>
                                </h3>
                                <div className="relative flex flex-wrap items-center my-2">
                                    <a href="#" className="inline-block text-[.925rem] font-medium leading-normal text-center align-middle cursor-pointer rounded-2xl transition-colors duration-150 ease-in-out text-light-inverse bg-light-dark border-light shadow-none border-0 py-2 px-5 hover:bg-secondary active:bg-light focus:bg-light" onClick={() => navigate('/create')}> 创建一个兴趣圈 </a>
                                </div>
                            </div>
                                    ...
                                    <table className="w-full my-0 align-middle text-dark border-neutral-200">
                                        <thead className="align-bottom">
                                            <tr className="font-semibold text-[0.95rem] text-secondary-dark">
                                                ...
                                                <th className="pb-3 text-end min-w-[50px]">加入</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {circles.map(circle => (
                                                ...
                                                <tr className="border-b border-dashed last:border-b-0">
                                                    ...
                                                    <td className="p-3 pr-0 text-end">
                                                        <button className="ml-auto relative text-secondary-dark bg-light-dark hover:text-primary flex items-center h-[25px] w-[25px] text-base font-medium leading-normal text-center align-middle cursor-pointer rounded-2xl transition-colors duration-200 ease-in-out shadow-none border-0 justify-center" id={circle.circle_id} onClick={() => navigate(`/circle/${circle.circle_id}`)}>
